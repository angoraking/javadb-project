...
import javlibrary_crawler.vendor.aws_ops_alpha.api as aws_ops_alpha
...
def deploy_config(check: bool = True):
    simple_config_project.deploy_config(
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.deploy_config.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def run_unit_test(check: bool = True):
    simple_python_project.run_unit_test(
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.run_code_coverage_test.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def build_doc(check: bool = True):
    ...
    simple_python_project.build_doc(
        ...
        pyproject_ops=pyproject_ops,
        check=check,
        step=javlibrary_crawler_project.StepEnum.build_documentation.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def deploy_versioned_doc(check: bool = True):
    ...
    simple_python_project.deploy_versioned_doc(
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.update_documentation.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def build_lambda_source(
    ...
    return javlibrary_crawler_project.build_lambda_source(
        pyproject_ops=pyproject_ops,
        verbose=verbose,
    )
...
def publish_lambda_layer(
    ...
    return javlibrary_crawler_project.publish_lambda_layer(
        semantic_branch_name=git_repo.semantic_branch_name,
        runtime_name=runtime.current_runtime_group,
        env_name=detect_current_env(),
        bsm_devops=boto_ses_factory.bsm_devops,
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.publish_lambda_layer.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def deploy_app(
    ...
    return javlibrary_crawler_project.deploy_app(
        semantic_branch_name=git_repo.semantic_branch_name,
        runtime_name=runtime.current_runtime_group,
        env_name=detect_current_env(),
        pyproject_ops=pyproject_ops,
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.deploy_cdk_stack.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def delete_app(
    ...
    if runtime.is_local:
        ...
        skip_prompt = True
    skip_prompt = True  # uncomment this if you always want to skip prompt
    return javlibrary_crawler_project.delete_app(
        semantic_branch_name=git_repo.semantic_branch_name,
        runtime_name=runtime.current_runtime_group,
        env_name=detect_current_env(),
        pyproject_ops=pyproject_ops,
        ...
        skip_prompt=skip_prompt,
        check=check,
        step=javlibrary_crawler_project.StepEnum.delete_cdk_stack.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def publish_lambda_version(
    ...
    env_name = detect_current_env()
    return javlibrary_crawler_project.publish_lambda_version(
        semantic_branch_name=git_repo.semantic_branch_name,
        runtime_name=runtime.current_runtime_group,
        env_name=detect_current_env(),
        bsm_workload=boto_ses_factory.get_env_bsm(env_name),
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.publish_new_lambda_version.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def run_int_test(check: bool = True):
    if runtime.is_local:
        ...
        wait = True
    javlibrary_crawler_project.run_int_test(
        semantic_branch_name=git_repo.semantic_branch_name,
        runtime_name=runtime.current_runtime_group,
        env_name=detect_current_env(),
        pyproject_ops=pyproject_ops,
        wait=wait,
        check=check,
        step=javlibrary_crawler_project.StepEnum.run_integration_test.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def create_config_snapshot(check: bool = True):
    simple_config_project.create_config_snapshot(
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.create_artifact_snapshot.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
...
def delete_config(check: bool = True):
    simple_config_project.delete_config(
        ...
        check=check,
        step=javlibrary_crawler_project.StepEnum.delete_config.value,
        truth_table=javlibrary_crawler_project.truth_table,
        url=javlibrary_crawler_project.google_sheet_url,
    )
