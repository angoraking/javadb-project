...
First, let's create a new project code skeleton using a sample project. It will prompt you to enter your project name and some configuration::
    ...
    # assume that you are currently at the ``./projects/javlibrary_crawler-project`` directory.
    python ../../cookiecutter/javlibrary_crawler.py
...
------------------------------------------------------------------------------
In this step, we focus on the core application code logic of the project. From this point, I assume that you are currently at the ``./projects/javlibrary_crawler-project`` (``./projects/${du_name}-project``) directory.

1. Create a feature branch: ``javlibrary_crawler/feature/${description}`` (``${du_name}/feature/${description}``) and start development.
...
10. Now you can merge the ``javlibrary_crawler/feature/${description}`` to ``main``.
...
1. Create a layer branch ``javlibrary_crawler/layer/${description}`` (``${du_name}/layer/${description}``).
...
4. Once you see a new layer is published, you can merge the ``javlibrary_crawler/layer/${description}`` to ``main``.
...
1. Create a lambda branch ``javlibrary_crawler/lambda/${description}`` (``${du_name}/lambda/${description}``).
...
6. Once you see the app is deployed to ``sandbox`` and the integration test is passed, you can merge the ``javlibrary_crawler/lambda/${description}`` to ``main``.
...
1. Create a release branch ``javlibrary_crawler/release`` (``${du_name}/release``). Don't do any application code change in this branch. If you have to, please roll back to the previous step to ensure that your change doesn't break the test.
...
- Create a cleanup branch: ``javlibrary_crawler/cleanup`` (``${du_name}/cleanup``). Don't do any application code change in this branch.
