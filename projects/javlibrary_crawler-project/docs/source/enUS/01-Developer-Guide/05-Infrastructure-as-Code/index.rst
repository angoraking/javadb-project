...
The ``cdk`` folder
------------------------------------------------------------------------------
The ``cdk`` folder is the CDK official convention where you should store your IAC declaration code and where you should run the ``cdk deploy`` command. However, for enterprise project, we should break down complicated logics into modules for better maintainability. The ``cdk/app.py`` script just import the declaration code from the ``javlibrary_crawler/iac/`` Python modules.


The ``javlibrary_crawler/iac/`` Python Modules
------------------------------------------------------------------------------
``javlibrary_crawler/iac/`` is the root folder of the infrastructure as code source code. Below are the list of important files related to Infrastructure as Code::

    javlibrary_crawler/iac # the root folder of the infrastructure as code source code
    javlibrary_crawler/iac/define/ # CDK stack declaration code
    javlibrary_crawler/iac/define/main.py # centralized CDK stack object, AWS resources are break down into sub-modules
    javlibrary_crawler/iac/define/iam.py # IAM related AWS resources
    javlibrary_crawler/iac/define/lbd.py # Lambda function related AWS resources
    javlibrary_crawler/iac/exports.py # CDK stack output exports for other projects to use
...
------------------------------------------------------------------------------
1. Working on ``javlibrary_crawler/iac/`` module to declare your CDK stack.
